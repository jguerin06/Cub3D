...
/*   Updated: 2024/10/19 16:39:56 by jguerin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int check_color(t_struct2 *map)
{
...
int	main(int argc, char **argv)
{
	...
	if (tmax(argv[1], &parsing, &map) == ERROR)
		return (-1); // ajouter fonction qui free les struct
	printf("test\n");
	ft_fill_info(&map, map.map);
	if (check_color(&map) == ERROR)
		return (ERROR); // gerer les leaks
	printf("caca\n");
	// check si ceiling et floor sont null
